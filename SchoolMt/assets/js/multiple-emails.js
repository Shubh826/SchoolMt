		...
		return this.each(function () {
		    ...
		    //$orig refers to the input HTML node
		        ...
		        if (settings.data!='') {
		                ...
		                var arr = settings.data.trim().replace(/^,|,$/g, '').replace(/^;|;$/g, '');
		                //Remove the double quote
		                arr = arr.replace(/"/g, "");
		                //Split the string into an array, with the space, comma, and semi-colon as the separator
		                arr = arr.split(/[\s,;]+/);
		                //var Emails = new Array(); //New array to contain the Emails
		                var Email = '';
		                for (var i = 0; i < arr.length; i++) {
		                    //Check if the email is already added, only if dupEmailCheck is set to true
		                    if (pattern.test(arr[i]) == true && settings.data!='') {
			...
			//}
		        ...
		        var $input = $('<input type="text" class="multiple_emails-input text-center" placeholder="Email" maxlength="50" />').on('keyup', function (e) { // input
			    //debugger
				...
				//if(event.which == 8 && input_length == 0) { $list.find('li').last().remove(); } //Removes last item on backspan with no input
